...
**Approach:** Single user prompt per interviewer with inline tone instruction.
...
`interviewer_lastname` = last whitespace-delimited token of `interviewer['name']`,
lowercased. Example: `"John Smith"` → `"smith"`,
`"Dr. Mary Jane Watson"` → `"watson"`.
...
- Without panel label:
  `data/job_packages/[role]/thankyou_[stage]_[lastname]_[date].txt`
  `data/job_packages/[role]/thankyou_[stage]_[lastname]_[date].docx`

`[date]` = `str(date.today())` at script start, consistent with project convention.

### Overwrite Protection

Before writing each interviewer's files, check if the `.txt` path exists. If so, prompt:
`"thankyou_[...].txt already exists. Overwrite? (y/n):"`. If `n`, skip both `.txt` and
`.docx` for that interviewer and print a skip notice. Continue to the next interviewer --
do not abort the run.

### .docx Construction

Uses `python-docx` directly (already a project dependency). A thank you letter is flat
prose -- no heading hierarchy, no tables, no numbered lists.

**Document structure:**

```
Title paragraph:    "Thank You -- [Interviewer Name], [Interviewer Title]"
                    Bold, 14pt, font consistent with project convention
Metadata paragraph: "Role: [role]  |  Stage: [stage]  |  Date: [date]"
                    Italic, 10pt, font consistent with project convention
Blank paragraph
Letter body:        Each paragraph of the generated letter as a separate
                    Paragraph element, normal weight, 12pt
```

**Page setup:**
- Paper size: US Letter (8.5 x 11 inches)
- Margins: 1 inch all sides
- Font: match the font used in `phase5_interview_prep.py` for body text

**Font resolution:** Before writing any .docx output, check the font constant or
default used in `phase5_interview_prep.py` and use the same value. Do not hardcode
a different font.

If .docx generation fails for any reason, print a warning and continue -- the .txt
file is the primary artifact and is already written at that point.

### Summary Block

Printed to terminal after all letters are processed:

```
Generated N thank you letters:
  data/job_packages/[role]/thankyou_[...].txt
  data/job_packages/[role]/thankyou_[...].docx
  ...

Skipped:
  [interviewer name] -- file exists, user declined overwrite
```

Skipped section omitted entirely if no interviewers were skipped.
...
| `test_docx_title_paragraph` | .docx title contains interviewer name and title |
| `test_docx_metadata_paragraph` | .docx metadata contains role, stage, and date |
| `test_docx_failure_continues` | .docx write failure → warning printed, run continues |
| `test_overwrite_declined` | Existing file + user enters n → skipped, next interviewer continues |
| `test_overwrite_accepted` | Existing file + user enters y → file overwritten |
| `test_summary_block_output` | Terminal summary lists generated and skipped files correctly |
