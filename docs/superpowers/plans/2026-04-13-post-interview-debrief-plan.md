...
> ⚠ REVIEW: `run_convert` reads `interview_date` and `produced_date` from the YAML and passes them directly to `build_output_filename`. YAML's `safe_load` will parse an unquoted date value (e.g., `interview_date: 2026-04-10`) as a `datetime.date` object, not a string -- causing `build_output_filename` to fail in the f-string. The fix is a one-line defensive cast on both fields before they are used: `interview_date = str(data['metadata']['interview_date'])` and `produced_date = str(data['metadata']['produced_date'])`. Apply this cast in `run_convert` immediately after `yaml.safe_load`. Add a corresponding test: write a draft YAML with unquoted date values and assert that `run_convert` produces a correctly named output file without error.
...
> ⚠ REVIEW: The spec requires a test for the `--interactive` draft conflict warning (`A draft already exists for [role]/[stage]...`). Confirm `test_run_interactive_draft_conflict_warning` is present in `tests/test_phase_debrief.py` and is included in this test run before marking this step complete. The test should assert both that the warning message is printed and that the session proceeds when the user confirms with `y`.
...
> ⚠ REVIEW: `get_followup_question` imports `strip_pii` from `scripts.utils.pii_filter` and calls it before every Claude API call. The plan does not verify that this function exists at that exact import path before Task 7 begins. If the function signature or module path has drifted, Task 7 fails at import rather than at a meaningful test. Before writing Task 7 code, confirm the import resolves: `python -c "from scripts.utils.pii_filter import strip_pii; print('OK')"`. If it fails, resolve the path before proceeding.
