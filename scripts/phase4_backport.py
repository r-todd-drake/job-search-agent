...
def extract_library_bullets(library_md_path: str) -> list:
    """Return all employer bullets from experience_library.md with metadata.

    Returns list of {text, theme, employer, line_number, sources}.

    Delegates structure parsing to scripts.utils.library_parser.parse_library
    (the canonical parser shared with the phase3 scripts) so this script never
    re-implements the library hierarchy and cannot drift out of sync with it.
    Summaries are intentionally ignored here.
    """
    employers, _summaries = parse_library(library_md_path, track_line_numbers=True)

    bullets = []
    for employer_name, employer_data in employers.items():
        for b in employer_data["bullets"]:
            bullets.append({
                "text": b["text"],
                "theme": b["theme"],
