...
1. Select the best bullets for each employer based on fit with this specific JD.
   - Tier 1 employers (shown as "Tier 1" in the list above): select 3-5 bullets each
   - Tier 2 employers (shown as "Tier 2"): select 1-3 bullets if relevant, 0 if not
   - Tier 3 employers (shown as "Tier 3"): select 0-1 bullets only if directly relevant
...
Do not include any other text. Follow the format exactly."""
    ...
    certs_line = cfg.get("resume_defaults", {}).get("certifications_line", "[certifications]")
...
- The Clearance & Certifications line must be EXACTLY:
  "Clearance & Certifications: {certs_line}"
...
- Use "Group 3" (not "Class III") when referring to UAS size classification
...
def main():
    ...
    # -- STAGE 4 --
        ...
        print(f"  5. Generate networking outreach after submitting:")
        print(f"     python -m scripts.phase6_networking --contact \"[name]\" --stage 1")
