...
from scripts.utils import domain_config
...
def _build_system_prompt():
    label = domain_config.get_label()
    return f"""You are an expert career coach writing professional networking messages \
for senior {label} professionals. Your messages are specific, \
...
def _build_inbound_prompt(contact: dict, candidate: dict) -> str:
    """Reply to an unsolicited inbound LinkedIn connection request (source LinkedIn-inbound).
    Thanks the contact and subtly invites them to share what led them to connect."""
    warmth = contact.get("warmth", "Cold")
    weight = (
        "warm and direct" if warmth.lower() in ("former colleague", "strong")
        else "professional and lightly curious"
    )

    return "\n".join([
        f"Write a brief reply from {os.getenv('CANDIDATE_NAME', '[CANDIDATE]')} "
        f"to {contact['contact_name']}, a {contact.get('title', 'professional')} "
        f"at {contact.get('company', 'their company')}, who has just sent the candidate "
        "a LinkedIn connection request that the candidate accepted.",
        "",
        "Candidate background:",
        _build_candidate_context(candidate),
        "",
        f"Warmth level: {warmth}",
        f"Notes on the relationship: {contact.get('notes') or 'No prior contact -- this is an unsolicited inbound connection.'}",
        "",
        "Requirements:",
        f"- Tone: {weight}",
        "- Thank them genuinely for connecting",
        "- Subtly invite them to share what led them to connect or what prompted the request "
        "-- make it feel natural and curious, not interrogating",
        "- Do not invent or guess why they reached out or any shared history",
        "- 2-3 sentences maximum",
        "- Warm and open -- no pressure, no role ask",
        "- No hollow openers",
    ])
...
def generate_inbound_message(contact: dict, candidate: dict, client=None) -> str:
    """Generate a reply to an inbound LinkedIn connection request.
    Pure function -- no file I/O. Injectable client for testing.
    Kept separate from generate_message so the stage-only validation stays intact."""
    if client is None:
        client = Anthropic()
    ...
    response = client.messages.create(
        ...
        max_tokens=1024,
        system=_build_system_prompt(),
        messages=[{"role": "user", "content": strip_pii(_build_inbound_prompt(contact, candidate))}],
    )
    return response.content[0].text
...
def _warn_if_source_mismatch(contact: dict) -> None:
    """Print a warning if --inbound is used for a contact whose source is not LinkedIn-inbound."""
    source = (contact.get("source") or "").lower()
    if source != "linkedin-inbound":
        print(
            f"Warning: {contact['contact_name']} has source '{contact.get('source') or '(none)'}', "
            "not 'LinkedIn-inbound'. Generating an inbound reply anyway."
        )
...
def _build_write_back(stage: int, role: str = None) -> dict:
    ...
    if stage == 2:
        updates = {"stage": 3, "status": "Activated"}
        if role:
...
def _run_inbound(contact: dict) -> None:
    """Generate an inbound-reply message, print it, and write back on confirm."""
    _warn_if_source_mismatch(contact)
    candidate = candidate_config.load()

    print(
        f"\n=== Inbound Reply | {contact['contact_name']} @ {contact.get('company', '')} "
        f"| {contact.get('warmth', '')} ===\n"
    )

    raw = generate_inbound_message(contact, candidate)
    print(raw)

    answer = input("\nDid you send this? (y/n): ").strip().lower()
    if answer == "y":
        update_contact(CONTACTS_TRACKER_PATH, contact["contact_name"], _build_write_back(1))
...
def main():
    ...
    parser.add_argument("--inbound", action="store_true",
                        help="Reply to an inbound LinkedIn connection request (source LinkedIn-inbound)")
    ...
    if args.stage is not None and args.inbound:
        parser.error("--stage and --inbound are mutually exclusive.")
    if args.stage is None and not args.inbound:
        parser.error("one of --stage or --inbound is required when using --contact")

    contact = find_contact(contacts, args.contact)

    if args.inbound:
        _run_inbound(contact)
        return
