...
def _build_system_prompt():
    ...
    return f"""You are an expert career coach specializing in {label}. \
You help senior professionals prepare for technical interviews.

You always:
- Ground all stories and claims in the candidate's confirmed experience only
- Use employer attribution in stories ("During my time at [Employer] as [Title]..." or
  "When I was at [Company] on [Program]...")
...
def _build_questions_hiring_manager():
    label = domain_config.get_label()
    return f"""
Generate exactly 4 questions for a candidate to ask at the end of a hiring manager interview
for a {label} role. The questions should signal program awareness,
...
4. The hiring manager's vision for where the technical effort (domain-specific methodology, architecture, or the relevant discipline) goes from here
...
def _build_questions_team_panel():
    label = domain_config.get_label()
    return f"""
Generate exactly 4 questions for a candidate to ask at the end of a team panel interview
for a {label} role. The questions should signal technical credibility,
process fluency, and peer-level awareness of where the hard work actually lives.

Question categories to cover:
1. Day-to-day working environment -- tooling cadence, process governance, or workflow specifics
...
def _build_questions_behavioral():
    label = domain_config.get_label()
    return f"""
Generate exactly 4 questions for a candidate to ask at the end of a behavioral interview
for a {label} role. The questions should signal self-awareness,
...
def _build_questions_technical_panel():
    label = domain_config.get_label()
    return f"""
Generate exactly 4 questions for a candidate to ask at the end of a technical panel interview
for a {label} role. The questions should signal architectural depth,
...
3. How the team approaches verification and validation for novel or emergent system
   behaviors -- signals {label} technical fluency
...
STAGE_PROFILES = {
    ...
    "hiring_manager": {
        ...
        "questions_prompt": _build_questions_hiring_manager,
    },
    "team_panel": {
        ...
        "questions_prompt": _build_questions_team_panel,
    ...
    "behavioral": {
        ...
        "questions_prompt": _build_questions_behavioral,
    },
    "technical_panel": {
        ...
        "questions_prompt": _build_questions_technical_panel,
        "peer_frame_prompt": _PEER_FRAME_INSTRUCTIONS,
    },
}
...
def extract_salary(jd_text, clearance_level=None):
    ...
    _clearance_note = f" and current {clearance_level} clearance" if clearance_level else ""
    ...
    return {
        ...
        'guidance': (
            ...
            f"  If asked: '${anchor_rounded:,.0f} based on my experience in this domain{_clearance_note}, "
            f"though I am open to discussing total compensation.'\n"
...
def generate_prep(client, role_data, interview_stage, output_txt_path, output_docx_path,
    ...
    questions_prompt = context_block + (_qp() if callable(_qp) else _qp)
