...
def parse_library(filepath, track_line_numbers=False):
    """
    Parse experience_library.md into structured data.
    Returns: (employers dict, summaries list)

    When track_line_numbers=True, each bullet dict gains a 1-based
    "line_number" field (the line the bullet text appears on). Off by
    default so phase3 JSON output is unchanged; phase4_backport opts in.
    """
    ...
    while i < len(lines):
        ...
        if stripped.startswith('- ') and current_employer and current_theme:
            ...
            if track_line_numbers:
                current_bullet["line_number"] = i + 1
            i += 1
            continue
