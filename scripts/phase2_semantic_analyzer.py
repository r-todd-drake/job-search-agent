...
def _build_system_prompt():
    ...
    return f"""You are an expert career coach and {label} recruiter
specializing in {label} talent. You analyze job descriptions against
...
End your response with this exact two-line block. No markdown, no emojis, no extra text after it:
FIT SCORE: X/10
RECOMMENDATION: PURSUE
(Replace X with the numeric score and use exactly one of: PURSUE, CONSIDER, SKIP.)
"""
    try:
...
_FIT_SCORE_RE = re.compile(r'\*{0,2}\s*fit\s+score\s*:\s*(\d+(?:\.\d+)?)', re.IGNORECASE)

def extract_fit_score(analysis_text):
    for line in analysis_text.split('\n'):
        m = _FIT_SCORE_RE.search(line)
        if m:
...
_RECOMMENDATION_RE = re.compile(
    r'(?:recommendation|verdict)\s*:\s*(PURSUE|CONSIDER|SKIP)', re.IGNORECASE
)

def extract_recommendation(analysis_text):
    for line in analysis_text.split('\n'):
        m = _RECOMMENDATION_RE.search(line)
        if m:
...
def main():
    ...
    all_jobs = []
    with open(JOBS_CSV, newline='', encoding='latin-1') as f:
    ...
    for i, job in enumerate(jobs_to_analyze, 1):
        ...
        report_lines.append("=" * 60)
        req_label = f"  Req: {req_number}" if req_number else ""
        report_lines.append(f"#{i}  [{status}]  {company} | {title}{req_label}")
    ...
    table_lines = []
    table_lines.append("")
    table_lines.append("=" * 116)
    table_lines.append("   COMBINED ANALYSIS SUMMARY")
    table_lines.append("=" * 116)
    table_lines.append(f"{'Rank':<5} {'St':<8} {'Company':<22} {'Title':<28} {'Req #':<14} {'Keyword':>8}  {'Sem':>5}  {'Action':<8}  {'Salary'}")
    table_lines.append("-" * 116)

    for i, r in enumerate(table_data, 1):
        company = trunc(r['company'], 22)
        title = trunc(r['title'], 28)
        req_number = trunc(r.get('req_number', ''), 14)
