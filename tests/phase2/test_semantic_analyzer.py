...
def test_analyze_job_max_tokens_leaves_room_for_full_analysis():
    """max_tokens must be high enough that the model can finish the full
    strengths/gaps analysis AND still emit the trailing FIT SCORE /
    RECOMMENDATION block. At 1000 tokens, verbose analyses were observed
    being cut off mid-sentence before reaching that block (see
    outputs/semantic_analysis_20260701_0833.txt), which made
    extract_fit_score/extract_recommendation fall back to 0/N/A."""
    from scripts.phase2_semantic_analyzer import analyze_job
    client = make_mock_client(MOCK_ANALYSIS)
    jd_text = FIXTURE_JD.read_text(encoding="utf-8")
    candidate_profile = "Senior systems engineer, TS/SCI, MBSE expert."
...
def test_system_prompt_uses_domain_label(monkeypatch):
    ...
    monkeypatch.setattr(dc, "_config", {"domain_label": "financial services"})

    from scripts.phase2_semantic_analyzer import _build_system_prompt
