        ...
        "stories": [
            {
                ...
                "notes": None,
                "source": "workshopped",
                "roles_used": ["Orbital_SE_IS"],
                "last_updated": "2026-04-15"
            },
            {
                "id": "meridian-autonomy-cross-domain",
                "title": "Cross-domain PDR",
                "tags": ["cross-functional", "technical-credibility"],
                "employer": "Meridian Autonomy",
                ...
                "notes": None,
                "source": "workshopped",
                "roles_used": ["Orbital_SE_IS", "DefenseCore_SE"],
                "last_updated": "2026-04-15"
            }
        ],
        "gap_responses": [
...
def test_load_tags_default_path_is_tracked_template(tmp_path, monkeypatch):
    """
    TAGS_PATH must default to templates/ (tracked script inputs), not data/ --
    data/ is gitignored personal space and must hold no tracked files.
    """
    monkeypatch.chdir(tmp_path)
    (tmp_path / "templates").mkdir()
    (tmp_path / "templates" / "interview_library_tags.json").write_text(
        json.dumps({"tags": ["leadership", "mbse"]}), encoding="utf-8"
    )

    tags = ilp.load_tags()

    assert tags == ["leadership", "mbse"]
...
def test_get_stories_filters_by_tag(tmp_path, monkeypatch):
    ...
    assert len(result) == 1
    assert result[0]["id"] == "talon-dynamics-mbse-bottleneck"
...
def test_get_stories_filters_by_role(tmp_path, monkeypatch):
    _write_library(tmp_path, _sample_library(), monkeypatch)
    result = ilp.get_stories(role="DefenseCore_SE")
    assert len(result) == 1
    assert result[0]["id"] == "meridian-autonomy-cross-domain"


def test_get_stories_filters_by_tag_and_role(tmp_path, monkeypatch):
    _write_library(tmp_path, _sample_library(), monkeypatch)
    # meridian-autonomy story has cross-functional tag and DefenseCore_SE role
    result = ilp.get_stories(tags=["cross-functional"], role="DefenseCore_SE")
    assert len(result) == 1
    assert result[0]["id"] == "meridian-autonomy-cross-domain"
...
def test_get_stories_tag_and_role_no_match(tmp_path, monkeypatch):
    _write_library(tmp_path, _sample_library(), monkeypatch)
    # mbse tag exists, but not for DefenseCore_SE
    result = ilp.get_stories(tags=["mbse"], role="DefenseCore_SE")
    assert result == []
...
def test_get_gap_responses_filters_by_role(tmp_path, monkeypatch):
    _write_library(tmp_path, _sample_library(), monkeypatch)
    result = ilp.get_gap_responses(role="Orbital_SE_IS")
    assert len(result) == 1
