...
def test_build_section2_prompt_injects_seed_block():
    ...
    seeds = [{
        "id": "story_001", "employer": "Talon Dynamics", "title": "Lead SE", "dates": "2022-2024",
    ...
    assert "Talon Dynamics" in prompt
...
def test_system_prompt_uses_domain_label(monkeypatch):
    ...
    monkeypatch.setattr(dc, "_config", {"domain_label": "product management"})

    from scripts.phase5_interview_prep import _build_system_prompt
    result = _build_system_prompt()

    assert "product management" in result
    assert "defense and aerospace" not in result


def test_questions_hiring_manager_uses_domain_label(monkeypatch):
    import scripts.utils.domain_config as dc
    monkeypatch.setattr(dc, "_config", {"domain_label": "product management"})

    from scripts.phase5_interview_prep import _build_questions_hiring_manager
    result = _build_questions_hiring_manager()

    assert "product management" in result
    assert "defense" not in result.lower()
    assert "MBSE" not in result


def test_questions_team_panel_uses_domain_label(monkeypatch):
    import scripts.utils.domain_config as dc
    monkeypatch.setattr(dc, "_config", {"domain_label": "product management"})

    from scripts.phase5_interview_prep import _build_questions_team_panel
    result = _build_questions_team_panel()

    assert "product management" in result
    assert "defense" not in result.lower()


def test_questions_behavioral_uses_domain_label(monkeypatch):
    import scripts.utils.domain_config as dc
    monkeypatch.setattr(dc, "_config", {"domain_label": "product management"})

    from scripts.phase5_interview_prep import _build_questions_behavioral
    result = _build_questions_behavioral()

    assert "product management" in result
    assert "defense" not in result.lower()


def test_questions_technical_panel_uses_domain_label(monkeypatch):
    import scripts.utils.domain_config as dc
    monkeypatch.setattr(dc, "_config", {"domain_label": "product management"})

    from scripts.phase5_interview_prep import _build_questions_technical_panel
    result = _build_questions_technical_panel()

    assert "product management" in result
    assert "defense" not in result.lower()
    assert "MBSE" not in result
    assert "V&V" not in result
