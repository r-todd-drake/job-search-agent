...
def test_parse_stories_extracts_employer_title_dates():
    paras = _story_paras([
        ...
        "Employer: Talon Dynamics | Systems Engineer | 2021-2023",
    ...
    assert stories[0]["employer"] == "Talon Dynamics"
...
def test_make_story_id_slugifies_employer():
    slug = wc._make_story_id("Talon Dynamics", ["mbse"])
    assert slug == "talon-dynamics-mbse"
...
def test_find_duplicate_story_matches_employer_and_tag():
    library = {
        "stories": [{"id": "talon-dynamics-mbse", "employer": "Talon Dynamics", "tags": ["mbse"],
                     "roles_used": ["OldRole"]}],
        "gap_responses": [], "questions": []
    }
    result = wc._find_duplicate_story(library, "Talon Dynamics", "mbse")
    assert result is not None
    assert result["id"] == "talon-dynamics-mbse"
...
def test_find_duplicate_story_employer_only_when_no_tags():
    ...
    result = wc._find_duplicate_story(library, "Talon Dynamics", "")
    assert result is not None
    assert result["id"] == "talon-dynamics-mbse"
...
def test_overwrite_merges_roles_used(tmp_path, monkeypatch):
    ...
    lib_path = _seed_library(tmp_path, library, monkeypatch)
    new_entry = {"id": "talon-dynamics-mbse", "employer": "Talon Dynamics", "tags": ["mbse"],
                 "roles_used": ["NewRole"]}
