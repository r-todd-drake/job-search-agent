...
def test_layer2_strips_pii_from_gaps_and_banned_sections(pii_values, monkeypatch):
    """
    Verifies run_layer2 strips PII from gaps_section and banned_section itself --
    these come from CANDIDATE_BACKGROUND.md and are not pre-stripped by callers.
    """
    ...
    gaps = f"No GitLab experience, noted by {pii_values['name']}."
    banned = f"Send corrections to {pii_values['email']}."
    run_layer2(client, "Clean cover letter text.", gaps_section=gaps, banned_section=banned)

    full_payload = str(client.messages.create.call_args)
    assert pii_values["name"] not in full_payload, "PII from gaps_section reached API payload"
    assert pii_values["email"] not in full_payload, "PII from banned_section reached API payload"


def test_system_prompt_uses_domain_label(monkeypatch):
    import scripts.utils.domain_config as dc
    monkeypatch.setattr(dc, "_config", {"domain_label": "software engineering"})

    from scripts.check_cover_letter import _build_system_prompt
