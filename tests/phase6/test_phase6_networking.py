...
def test_write_back_stage2_includes_role():
    ...
    assert updates["status"] == "Activated"
...
INBOUND = {**COLD, "source": "LinkedIn-inbound", "notes": ""}


def test_inbound_prompt_thanks_for_connecting():
    prompt = pn._build_inbound_prompt(INBOUND, CANDIDATE_FIXTURE)
    assert "thank" in prompt.lower()
    assert "connect" in prompt.lower()


def test_inbound_prompt_asks_why_they_connected():
    prompt = pn._build_inbound_prompt(INBOUND, CANDIDATE_FIXTURE)
    lowered = prompt.lower()
    assert "what led them to connect" in lowered or "what prompted" in lowered


def test_inbound_prompt_has_no_role_ask():
    prompt = pn._build_inbound_prompt(INBOUND, CANDIDATE_FIXTURE)
    assert "no role ask" in prompt.lower()
    assert "referral" not in prompt.lower()


def test_inbound_prompt_has_no_warmth_placeholder():
    prompt = pn._build_inbound_prompt(
        {**INBOUND, "warmth": "Acquaintance"}, CANDIDATE_FIXTURE
    )
    assert "[HOW YOU KNOW THIS PERSON]" not in prompt
    assert "[WHERE YOU WORKED TOGETHER]" not in prompt


def test_inbound_prompt_warmth_variants_differ():
    cold = pn._build_inbound_prompt({**INBOUND, "warmth": "Cold"}, CANDIDATE_FIXTURE)
    strong = pn._build_inbound_prompt({**INBOUND, "warmth": "Strong"}, CANDIDATE_FIXTURE)
    assert cold != strong


# ==============================================
# generate_inbound_message
# ==============================================

def test_generate_inbound_message_calls_api():
    client = _mock_client("Thanks so much for connecting.")
    result = pn.generate_inbound_message(INBOUND, CANDIDATE_FIXTURE, client=client)
    assert client.messages.create.called
    assert isinstance(result, str)


# ==============================================
# _warn_if_source_mismatch
# ==============================================

def test_source_mismatch_warns_on_non_inbound_source(capsys):
    pn._warn_if_source_mismatch({**COLD, "source": "LinkedIn search"})
    captured = capsys.readouterr()
    assert "Warning" in captured.out


def test_source_mismatch_silent_on_inbound_source(capsys):
    pn._warn_if_source_mismatch({**COLD, "source": "LinkedIn-inbound"})
    captured = capsys.readouterr()
    assert "Warning" not in captured.out


def test_system_prompt_uses_domain_label(monkeypatch):
    import scripts.utils.domain_config as dc
    monkeypatch.setattr(dc, "_config", {"domain_label": "clinical healthcare management"})

    from scripts.phase6_networking import _build_system_prompt
    result = _build_system_prompt()

    assert "clinical healthcare management" in result
    assert "defense" not in result.lower()
