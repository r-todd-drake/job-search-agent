...
See context/INTERFACE_MAP.md for complete CLI specs, public function signatures, file I/O per script, and shared module APIs. Read this before working with any script's CLI or shared module functions.
...
5. If the change affects CLI args, public/exported function signatures, file I/O paths, or JSON schemas — update the relevant `context/*.md` file as part of the solution, and end the response with a `## Project Files to Sync` section listing each changed file and a one-line summary of what changed. Renaming or restructuring an internal helper does NOT trigger this rule.
...
## Writing Rules — Commit Messages and PR Descriptions
Writing rules, from Orwell, 1946. These govern git commit messages and pull request
descriptions written by Claude Code. Never touch code or technical terms; swap in
everyday words only where precision survives.
...
Review every commit message and PR description against these rules before delivering.