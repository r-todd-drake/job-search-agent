...
| --------------------- | ------------------------------------------------------------------------------------------------------------- | --------------- |
| `init_candidate.py`   | Guided terminal wizard for populating `context/candidate/candidate_config.yaml` from scratch; 10 sections: education, certifications, military, clearance, skills, gaps, employers, resume defaults, style rules; validates existing config before overwriting | — |
...
| `phase4_interactive.py`   | Interactive resume stage runner — role selection, Stage 1 generation, Stage 2 copy (in progress) | `--role`                                     |
...
| ------------------------------ | ------------------------------------------------------------------------------------- | ---------------------------------- |
| `config.py`                    | Shared path and model constants (`JOBS_PACKAGES_DIR`, `EXPERIENCE_LIBRARY_JSON`, `RESUMES_DIR`, etc.) | phase4_interactive and other scripts |
