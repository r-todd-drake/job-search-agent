...
- __Source-of-truth__ — this is where edits are made; other files are derived from it
- __Derived__ — generated by a script; never edit directly
...
__Fallback behavior:__ `phase4_resume_generator.py --stage 4` reads `stage4_final.txt` if it exists; otherwise falls back to `stage2_approved.txt`. `phase5_thankyou.py` follows the same fallback.
