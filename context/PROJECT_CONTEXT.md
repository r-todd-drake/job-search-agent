...
- **564 mock tests** across utils, phases 1–6. All passing as of 03 Jun 2026.
...
### Phase 6 — Status vocabulary write-back (03 Jun 2026)
`scripts/phase6_networking.py`:
- `_build_write_back()` stage 2 now writes `status → 'Activated'` so the tracker reflects that a referral ask has been sent
- example_data tracker corrected: stage 3 contacts use `Activated` (not `Active`) to match defined vocabulary
...
### phase2_semantic_analyzer.py — output enhancements (14 May 2026)
`scripts/phase2_semantic_analyzer.py`:
- `req_number` from `jobs.csv` now surfaces in both output locations:
  - Per-role body header: appended as `  Req: {req_number}` on the title line (suppressed when blank)
  - Combined Analysis Summary table: new `Req #` column (14 chars, truncated) between Title and Keyword; table separators widened to 116 chars
- `extract_fit_score()` bug fix: replaced `line.startswith('FIT SCORE:')` with compiled regex `_FIT_SCORE_RE` that handles Claude's markdown bold output (`**FIT SCORE: 6/10**`), case variation, and decimal scores (`8.5/10` → `int(round(float(...)))`)

### Domain generalization — B1/B2/B3 (26 May 2026)
Hardcoded defense/clearance terms extracted from system prompts into a configurable domain layer:
- `scripts/utils/domain_config.py` — singleton loader (`load()`, `get_label(key, default)`) reads `context/domain/domain_config.yaml`; gracefully returns the default string when the file is absent
- `context/domain/domain_config.example.yaml` — blank template for domain configuration (tracked)
- `context/domain/domain_config.yaml` — personal domain labels (gitignored)
- B2 refactor: `check_resume.py`, `check_cover_letter.py`, `phase2_semantic_analyzer.py`, `phase6_networking.py`, `phase5_interview_prep.py` — all system prompts now call `get_label()` instead of hardcoding domain-specific strings
- B3 refactor: `phase5_interview_prep.py` question-set prompts generalized for non-defense users via domain labels
- 537 mock tests passing post-merge (10 pre-existing live-only failures unaffected)

### Onboarding artifacts — C1/C2 (26 May 2026)
New artifacts for first-time setup and Claude.ai session onboarding:
- `project_setup.md` (root) — 88-line guided Claude.ai onboarding prompt; provides a structured context-load sequence for web chat sessions
- `scripts/init_candidate.py` — 10-section guided terminal wizard for populating `context/candidate/candidate_config.yaml` from scratch; sections: education, certifications, military, clearance, skills, gaps, employers, resume defaults, style rules; validates existing config before overwriting
- `tests/test_init_candidate.py` — 19 Tier 1 mock tests covering all 10 collect functions, save_config validation, and run_wizard orchestration
- Bug fix applied during TDD: `save_config()` accepts an explicit `config_path` parameter so the monkeypatch works correctly in worktree environments
